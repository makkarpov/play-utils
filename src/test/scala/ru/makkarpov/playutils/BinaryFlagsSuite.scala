...
import ru.makkarpov.playutils.TestUtils._
...
class BinaryFlagsSuite extends FunSuite {
  object MyPostgresDriver extends ExPostgresProfile with BinaryFlagsSupport with DatabaseProvider {
    override val api = MyAPI

    object MyAPI extends API with BinaryFlagsImplicits {
  ...
  implicit val testFlagsType = binaryFlagsType(TestFlags)
  import TestFlags.value2valueSet
  val db = MyPostgresDriver.database
